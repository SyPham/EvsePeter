...
export class H2DiscountCodeManagementComponent extends BaseComponent implements OnInit {
  ...
  model: CouponManagement = {} as CouponManagement;
  ...
  loadLang() {
    this.translate.get('CouponManagement').subscribe( functionName => {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}CouponManagement/LoadData?lang=${lang}`,
  ...
  cancel() {
    this.model = {} as CouponManagement;
    this.audit = {}
  }
  rowSelected(args) {
  ...
  fnEdit(data) {
    let link = `/h-app/h2/action/${data.id}`;
    this.router.navigateByUrl(link)
  }
  fnAdd() {
    let link = `/h-app/h2/action/${0}`;
    this.router.navigateByUrl(link)
  }
  fnSearch() {
