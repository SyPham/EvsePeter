...
@Component({
  selector: 'app-sitev2-action',
  templateUrl: './sitev2-action.component.html',
  styleUrls: ['./sitev2-action.component.scss']
})
export class Sitev2ActionComponent implements OnInit {
  loading
  model: Site = <Site>{};
  file: any;
...
loadlandlord() {
    const accessToken = localStorage.getItem("token");
   this.landlordData = new DataManager({
      url: `${environment.apiUrl}LandLord/GetDataDropdownlist`,
      adaptor: new UrlAdaptor(),
      headers: [{ authorization: `Bearer ${accessToken}` }],
    })
